...
export function RuntimePanel() {
  ...
  const load = useCallback(async () => {
    try {
      const [s, srv, cp] = await Promise.all([
  ...
  const handleDiscover = async () => {
    setLoading(true);
    try {
      const result = await apiGet<DiscoverResponse>("/api/runtime/opencode/discover");
  ...
  const handleAttach = async (host: string, port: number) => {
    setLoading(true);
    try {
      await apiPost("/api/runtime/opencode/attach", { host, port });
  ...
  const handleTest = async (host: string, port: number) => {
    setLoading(true);
    try {
      const result = await apiPost<{ok: boolean; is_running: boolean; message: string}>("/api/runtime/opencode/test", { host, port });
      flash(result.is_running ? `✓ ${host}:${port} is running` : `✗ ${host}:${port} unreachable`, result.is_running ? "success" : "error");
    } catch (err) {
  ...
  const handleIgnore = async (host: string, port: number) => {
    setLoading(true);
    try {
      await apiPost("/api/runtime/opencode/ignore", { host, port });
  ...
  const handleAssign = async (host: string, port: number, role: string) => {
    ...
    try {
      await apiPost("/api/runtime/opencode/attach", { host, port });
      await apiPost(`/api/runtime/bindings/${role}`, { host, port });
