...
function highlightCode(text: string): string {
  ...
  if (fenced) {
    ...
    const code = fenced[2];
    return `<div class="my-1 rounded overflow-hidden" style="border:1px solid var(--border)"><div class="px-2 py-0.5 text-[9px] uppercase tracking-wider" style="background:var(--bg-raised);color:var(--text-secondary);border-bottom:1px solid var(--border)">${lang}</div><pre class="p-2 text-[11px] whitespace-pre-wrap" style="background:var(--bg-base);color:var(--purple)">${code.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</pre></div>`;
  }
  return text.replace(/`([^`]+)`/g, '<code class="px-1 rounded text-[11px]" style="background:var(--bg-raised);color:var(--purple)">$1</code>');
}
        ...
        s.type === "block" && s.block
          ...
          ? (
            <div
              ...
              dangerouslySetInnerHTML={{ __html: highlightCode(s.value) }}
            />
          )
          : null
      )}
...
function ReadOutput({ ev }: { ev: EvTool }) {
  ...
  return (
    <div className="ml-6 mt-0.5 space-y-1">
      ...
      {ev.output && (
        <pre className="text-[10px] px-2 py-1 rounded overflow-x-auto whitespace-pre-wrap" style={{ color: "var(--green)", background: "var(--green-bg)" }}>
          {ev.output}
        </pre>
      )}
      {ev.error && (
        <pre className="text-[10px] px-2 py-1 rounded whitespace-pre-wrap" style={{ color: "var(--red)", background: "var(--red-bg)" }}>
          {ev.error}
        </pre>
      )}
    </div>
  ...
  return (
    <div className="ml-6 mt-0.5 space-y-1">
      {Object.keys(ev.input).filter(k => k !== "description" && k !== "desc").length > 0 && (
        <pre className="text-[10px] px-2 py-1 rounded overflow-x-auto" style={{ color: "var(--text-secondary)", background: "var(--bg-raised)" }}>
          {JSON.stringify(
