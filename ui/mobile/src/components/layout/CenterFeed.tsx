...
import { AgentConfig, AgentStream, SettingsToggle, ChatPanel } from "./CenterFeedComponents";
...
export function CenterFeed({ agents, activeAgent, onSelectAgent, onRefresh }: Props) {
  ...
  useEffect(() => {
    ...
    const onSessionSwitched = () => loadSession();
    window.addEventListener("task-hounds-session-switched", onSessionSwitched);
    return () => {
      clearInterval(id);
      window.removeEventListener("task-hounds-session-switched", onSessionSwitched);
    };
  }, []);
  ...
  return (
    <main className="flex-1 flex flex-col min-w-0 min-h-0" style={{ background: "var(--bg-base)" }}>
      ...
      {/* Stream */}
      {agent ? (
        <AgentStream key={`${sessionId}-${agent.name}`} agent={agent} sessionId={sessionId} onClear={onRefresh} onStop={onRefresh} />
      ) : (
